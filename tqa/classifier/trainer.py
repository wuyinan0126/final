...
from tqa.classifier.data import ClassifierDataset, ClassifierSampler
...
def prepare_args(parser):
    parser.register('type', 'bool', c_utils.str2bool)
    parser.register('type', 'list', c_utils.str2int_list)
    ...
    preprocess.add_argument('--uncased-text', type='bool', default=DEFAULTS['uncased_text'],
                            help='是否将文本中的单词小写化')
    ...
    model_arch.add_argument('--embedding-size-t', type=int, default=DEFAULTS['embedding_size_t'],
    ...
    model_arch.add_argument('--hidden-size', type=int, default=DEFAULTS['hidden_size'],
                            help='RNN单元隐含层向量大小')
    model_arch.add_argument('--num-layers-t', type=int, default=DEFAULTS['num_layers_t'],
                            help='RNN text classifier层数')
    ...
    model_arch.add_argument('--num-layers-c', type=int, default=DEFAULTS['num_layers_c'],
                            help='CNN Linear层数')
    model_arch.add_argument('--num-classes', type=int, default=DEFAULTS['num_classes'],
                            help='类别总数')
    model_arch.add_argument('--bidirectional', type='bool', default=DEFAULTS['bidirectional'],
                            help='是否使用双向RNN')
    # ------------------------------------------------------------------------------
    # 特征选择相关
    # ------------------------------------------------------------------------------
...
def train(args):
    ...
    r_utils.set_environment(use_cuda, args.gpu_device, args.random_seed)
    # ------------------------------------------------------------------------------
    # 数据处理相关
    # ------------------------------------------------------------------------------
    logger.info('-' * 100)
    # train_examples和dev_examples区别是是否skip_no_answer
    train_examples = c_utils.load_parsed_data(args.train_data_path)
    logger.info('Number of train data = %d' % len(train_examples))

    dev_examples = c_utils.load_parsed_data(args.dev_data_path)
    ...
    if args.checkpoint and os.path.isfile(model_path + '.checkpoint'):
        ...
        model, start_epoch = r_utils.load_checkpoint(model_checkpoint_path)
    else:
        # 从以前训练好的模型开始训练，模型状态初始化使用训练好的模型
        if args.pretrained_model_path:
            logger.info('Training model from pretrained model...')
            model = r_utils.load_model(args.pretrained_model_path, new_args=None, model_type='classifier')
            logger.info('Expanding dictionary for new data...')
            # 得到examples和glove词典中都出现的单词集合
            valid_words = c_utils.load_valid_words(args, train_examples + dev_examples)
            new_chars = c_utils.load_chars(train_examples + dev_examples)
        ...
        # 从头开始训练，模型状态初始化使用随机方式
            ...
            model = r_utils.init_model_from_scratch(args, train_examples, dev_examples, model_type='classifier')

        # 微调问题中出现频率top_k的单词的embeddings
        if args.tune_top_k > 0:
            logger.info('-' * 100)
            logger.info('Counting top %d most frequent words' % args.tune_top_k)
            top_text_words = c_utils.get_top_text_words(train_examples, model.words, args.tune_top_k)
            for word in top_text_words[:5]:
                logger.info(word)
            logger.info('...')
            for word in top_text_words[-6:-1]:
                logger.info(word)
            # top_question_words ~= [('a', 5), ('b', 4), ('c', 3)]
            model.tune_embeddings([word[0] for word in top_text_words])
    ...
    train_dataset = ClassifierDataset(train_examples, model, single_class=True)
    # 采样器中的example indices顺序根据example中的文档长度和问题长度排序
    train_sampler = ClassifierSampler(train_dataset.lengths(), args.batch_size, shuffle=True)
    ...
    train_loader = DataLoader(
        ...
        collate_fn=c_utils.batchify,
        pin_memory=use_cuda,
    )
    dev_dataset = ClassifierDataset(dev_examples, model, single_class=False)
    dev_sampler = ClassifierSampler(dev_dataset.lengths(), args.dev_batch_size, shuffle=False)
    ...
    train_states = {'timer': r_utils.Timer(), 'epoch': 0, 'best_validation': 0}
    for epoch in range(start_epoch, args.num_epochs):
        train_states['epoch'] = epoch

        # 开始训练
        train_loss = r_utils.AverageMeter()
        epoch_timer = r_utils.Timer()
        ...
        # 每个epoch结束后评估
        c_utils.validate(train_loader, model, train_states, type='train')
        result = c_utils.validate(dev_loader, model, train_states, type='dev')
