...
class QuestionIndexView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        noans = Question.objects.order_by('-pub_date').filter(answer__isnull=True).select_related('user').annotate(
        ...
        context['noans'] = noans
        context['reward'] = Question.objects.order_by('-reward').filter(reward__gte=1).annotate(
            num_answers=Count('answer', distinct=True),
            num_question_comments=Count('questioncomment', distinct=True)
        )[:10]
        ...
        context['tags'] = Tag.objects.filter(taggit_taggeditem_items__in=items).order_by('-id').distinct()[:10]
        return context
...
class QuestionsSearchView(QuestionIndexView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(QuestionsSearchView, self).get_context_data(*args, **kwargs)
        context['totalcount'] = Question.objects.count
        context['anscount'] = Answer.objects.count
        context['noans'] = Question.objects.order_by('-pub_date').filter(answer__isnull=True).annotate(
            num_answers=Count('answer', distinct=True),
            num_question_comments=Count('questioncomment', distinct=True)
        )[:10]
        context['reward'] = Question.objects.order_by('-reward').filter(reward__gte=1).annotate(
